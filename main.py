...
def generate_troubleshooting_report():
    ...
    problem = input("Enter the problem you diagnosed: ").strip()
    findings = input("Enter the main possible cause: ").strip()
    action = input("Enter the recommended action: ").strip()

    report_lines = []

    report_lines.append("PC TROUBLESHOOTING REPORT")
    ...
    report_lines.append(f"Possible Cause: {findings}")
    report_lines.append(f"Recommended Action: {action}")
    report_lines.append("")

    report_lines.append("Generated by PC Troubleshooting Tool")

    filename = "troubleshooting_report.txt"

    with open(filename, "w") as file:
        file.write("\n".join(report_lines))

    print(f"\nTroubleshooting report created successfully: {filename}")
...
def main():
    while True:
        ...
        print("10. Generate Troubleshooting Report")
        print("11. Exit")
        choice = input("\nSelect a problem: ")
